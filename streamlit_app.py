...
recipes_df = load_csv_safe("recipes.csv")  # only once, safely

# Calorie catalogue (Tamil Nadu foods)
calorie_df_raw = load_csv_safe("tamil_nadu_foods.csv")
calorie_df = None
if calorie_df_raw is not None:
    # rename to consistent, mixed-case headers expected by UI below
    rename_map = {}
    ...
    # normalize calories header variants to a single column
...
def get_nutrition_info(item_name: str):
    ...
    def _find_in(df):
        ...
        if "name" not in df.columns:
            ...
            if "name" not in df.columns:
                return None
        mask = df["name"].astype(str).str.lower().str.contains(re.escape(item_name), na=False)
        match = df[mask]
...
def processed_img(img_path: str):
    if _model is None:
        raise RuntimeError("Model FV2.h5 not loaded. Place the model file next to the script.")
...
def show_nutrition_charts(item_name, nutrition):
    ...
    # nutrition may be a Series or dict; both are fine with pd.Series
    ...
    nutri_df["Value"] = pd.to_numeric(nutri_df["Value"], errors="coerce").fillna(0)
    nutri_df["% of RDI"] = [
        round((v / rdi[n]) * 100, 2) if rdi[n] > 0 else 0 for v, n in zip(nutri_df["Value"], nutri_df["Nutrient"])
    ]
...
def recommend_recipes(item_name: str, top_n: int = 6):
    ...
    # Expect 'food' & 'recipe' (both already lower due to _standardize_columns)
...
# ===================== Streamlit App =====================
def run():
    st.set_page_config(page_title="Fruit & Vegetable Classifier", layout="wide")
        ...
        if img_file:
            ...
            try:
                ...
                if nutrient_info_df is not None:
                    required = {"nutrient", "description"}
                    if required.issubset(set(nutrient_info_df.columns)):
                        st.subheader("ℹ️ Nutrient Information")
                        for _, row in nutrient_info_df.iterrows():
                            st.markdown(f"**{row['nutrient']}** → {row['description']}")
    ...
    with tab2:
        ...
        item_query = st.text_input("📝 Type fruit or vegetable:", key="search_text")
        all_items = sorted(fruits + vegetables)
        dropdown_choice = st.selectbox("🔽 Select fruit/vegetable:", [""] + all_items, key="search_dropdown")
    ...
    # -------------------- TAB 3: Meal Planner (Nutrients) --------------------
    with tab3:
        st.subheader("🍽️ Build Your Meal Plate (Nutrients)")
        item_list_text = st.text_input("📝 Type items (comma-separated):", key="meal_text")
        multiselect_items = st.multiselect("🔽 Choose items:", sorted(fruits + vegetables), key="meal_multi")

        selected_items = []
        if item_list_text:
            selected_items += [i.strip() for i in item_list_text.split(",") if i.strip()]
        ...
        if selected_items:
            ...
            for item in selected_items:
                ...
                for n in main_nutrients:
                    try:
                        total_nutrition[n] += float(pd.to_numeric(pd.Series(nutrition).get(n, 0), errors="coerce"))
                    except Exception:
            ...
            meal_df = pd.DataFrame({
                ...
                "% of RDI": [round((total_nutrition[n] / rdi[n]) * 100, 2) if rdi[n] > 0 else 0 for n in main_nutrients],
            })
            st.subheader("🥗 Nutrition Summary for Selected Items")
            st.dataframe(meal_df, use_container_width=True)
            plot_chart(meal_df["Nutrient"].tolist(), meal_df["% of RDI"].tolist(), "Total Meal — % of Daily Intake", "meal_total_rdi")
    ...
    # -------------------- TAB 4: Calorie Planner --------------------
    with tab4:
        st.subheader("🍛 Build Your Meal Plate (Calories) — Veg / Non-Veg")
        if calorie_df is None:
            st.warning("`tamil_nadu_foods.csv` not found or missing required columns.")
        else:
            search = st.text_input("📝 Filter dishes (optional):", key="calp_text")

            cat_col = "Category" if "Category" in calorie_df.columns else None
            category = st.radio("Category:", ["All", "Veg", "Non-Veg"], horizontal=True, key="cal_cat_radio")

            df = calorie_df.copy()
            if "Calorie Values (kcal)" in df.columns:
                df["Calorie Values (kcal)"] = pd.to_numeric(df["Calorie Values (kcal)"], errors="coerce")

            if category != "All" and cat_col:
                df = df[df[cat_col].astype(str).str.strip().str.lower() == category.lower()]

            fi_col = "Food Items" if "Food Items" in df.columns else None
            if search and fi_col:
                df = df[df[fi_col].astype(str).str.contains(search, case=False, na=False)]

            single_select = None
            if fi_col:
                all_foods = [""] + sorted(df[fi_col].dropna().astype(str).unique().tolist())
                single_select = st.selectbox("🔽 Pick one dish to add:", all_foods, key="cal_single")
            ...
            chosen = st.multiselect("Choose items:", options=options, default=chosen_default, key="cal_item_multiselect")
            ...
            if chosen:
                ...
                for opt in chosen:
                    key = f"serv_{opt}"
                    servings[opt] = st.number_input(opt, min_value=1, max_value=100, value=1, step=1, key=key)
                ...
                for opt in chosen:
                    ...
                    count = servings[opt]
                    total_kcal = (kcal if pd.notna(kcal) else 0) * count if kcal is not None else 0
                    rows.append({
                        "Food Item": name,
                ...
                total_calories = float(result_df["Total Calories"].sum()) if not result_df.empty else 0.0
                ...
                html = tpl.safe_substitute(labels=json.dumps(labels_pie), values=json.dumps(values_pie))
                st.components.v1.html(html, height=560)

    # -------------------- TAB 5: Mood Food --------------------
    with tab5:
        st.subheader("⚡ Mood-based Food Recommendation")
        mood_radio = st.radio("Pick your mood:", list(mood_food_map.keys()), horizontal=True, key="mood_radio")
        ...
        if effective_mood:
            ...
            # ================= Show Foods in Cards =================
            cols = st.columns(3)
            for idx, item in enumerate(mood_food_map[effective_mood][:6]):
                with cols[idx % 3]:
                    st.markdown(
